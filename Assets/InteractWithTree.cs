...
public class InteractWithTree : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0) && objectCanvas.activeInHierarchy && onScreenManager.axeToolSelected && onScreenManager.circularSlider.value == 0)
        {
            onScreenManager.fillSlider = true;
            onScreenManager.farmingResourceSound.Play();
        }
        else if (Input.GetKeyDown(KeyCode.Mouse0) && objectCanvas.activeInHierarchy && !onScreenManager.axeToolSelected)
        {
            onScreenManager.lerpTip = true;
            onScreenManager.tipText.text = onScreenManager.availableTexts[5];
            onScreenManager.errorSound.Play();
        }

        //if slider is now full and right tool is selected...
        if (onScreenManager.fillSlider && onScreenManager.axeToolSelected)
        {
    ...
    void HitRay()
    {
        ...
        if (Physics.Raycast(mainCamera.transform.position, mainCamera.transform.forward, out hit, range))
        {
            ...
            if (myStatement && onScreenManager.woodResourceAmmount < 990 && onScreenManager.exhaustedSlider.value > 5f)
            {
                ...
                onScreenManager.selectedImage = onScreenManager.woodImage;
                onScreenManager.resourceSound.Play();
            }
            else if (myStatement && onScreenManager.woodResourceAmmount <= 990 && onScreenManager.exhaustedSlider.value < 5f)
            {
                onScreenManager.lerpTip = true;
